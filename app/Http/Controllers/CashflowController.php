    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($cashflows)
                ->addIndexColumn()
                ->addColumn('nominal', function ($cashflow) {
                    return 'Rp' . number_format($cashflow->nominal, 2, ',', '.');
                })
                ->addColumn('type', function ($cashflow) {
                    return ucfirst($cashflow->type); // Format type to ucfirst
                })
